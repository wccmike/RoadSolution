$(function(){
	...
	LBMap.prototype.init = function(mid){
		// 百度地图API功能
		var map = new BMap.Map(mid,{mapType:BMAP_NORMAL_MAP});// 创建地图实例
	...
	LBMap.prototype.addMark = function(pt, setting){
		...
		if(setting.title != null && setting.title != ""){
			...
			label.setStyle({"border": "0px", "border-radius": "4px"});
	...
	// 绘制圆
	 LBMap.prototype.addCircle = function(pt, r){
		  ...
		  var circle = new BMap.Circle(point, r, {strokeColor:"", strokeWeight:2, strokeOpacity:0.5, fillOpacity: 0.2, fillColor:""}); 
	...
	LBMap.prototype.addPolyline = function(pts, icon){
		...
		var polyline = new BMap.Polyline(points, {strokeColor:"green", strokeWeight:2}); 
	...
	function addMarks(map){
		for(var i=0; i<2; i++){
			...
			var marker = map.addMark([x, y], {title: "垃圾处理" + (i + 1), icon: "images/person3.png"});
			




			marker.addEventListener(function(e){
				 //获取人员信息
				var mark = e.target;
		...
		for(var i=0; i<2; i++){
			...
			var y = 0;
			if(i % 4 == 0){
				x = parseFloat(lng.toFixed(2))+0.05 ;
				y = parseFloat(lat.toFixed(2));
			}else if(i % 4 == 1){
				x = parseFloat(lng.toFixed(2)) +0.03;
				y = parseFloat(lat.toFixed(2)) ;
			}else if(i % 4 == 2){
				x = parseFloat(lng.toFixed(2)) +0.04;
				y = parseFloat(lat.toFixed(2)) ;
			}else{
				x = parseFloat(lng.toFixed(2))+0.05;
				y = parseFloat(lat.toFixed(2)) ;
			...
			var marker = map.addMark([x, y], {title: "井盖处理" + (i + 1), icon: "images/person3.png"});
	...
	function selectPersonTrack(mark){
		...
		setTimeout(function(){
			...
			var marker = ptMap.addMark([cpt.lng, cpt.lat], {title: "路长", icon: "assets/images/person.png"});
